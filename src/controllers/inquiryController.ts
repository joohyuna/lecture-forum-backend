...
const getInquiryList = async (req: AuthRequest, res: Response) => {
    try {
        ...
        console.error(error);
        res.status(500).send({
            message: "문의 목록 조회중 서버 오류가 발생되었습니다. ",
        });
    }
};
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message === "NOT_FOUND_INQUIRY") {
                res.status(404).json({
                    message: "문의글을 찾을수 없습니다.",
                });
                return;
            } else if (error.message === "NOT_YOUR_INQUIRY") {
                res.status(403).json({
                    message: "문의글 수집에 대한 권한이 없습니다.",
                });
                return;
            } else if (error.message === "ALREADY_ANSWERED") {
                res.status(403).json({
                    message: "이미 답변이 달린 문의글은 수정할수 없습니다.",
                });
                return;
            }
        }
...
const deleteInquiry = async (req: AuthRequest<{ inquiryId: string }>, res: Response) => {
    try {
        ...
        if (inquiry.answer) {
            res.status(403).json({
                message: "답변이 존재하는 문의글로 삭제할 수 없습니다.",
            });
            return;
        }
