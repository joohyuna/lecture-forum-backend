...
// Request라는 Express기본 타입은 Generic Type임
// 그렇기 때문에 우리가 Request 타입을 쓸때 Request<{id: string}>라고
// 쓰는게 가능했었음
...
// 그래서 이 미들웨어 에서 return을 치면 미들웨어에서 절차가 끝내버림,
// 다음 함수를 실행하려면 next()를 호출 해야함
...
export const authenticate = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // 2. 그 Authorization의 값이 `Bearer 뭐뭐뭐로 들어오는지, 그 모모모가 있는지 그 값을 받아옴
        // authorization 프로퍼티의 값을 타입은 string 이기 때문에 startWith()메서드를 사용해서 확인
        // startsWith() 메서드: string 타입에서 사용 가능. 매개변수에 집어 넣어줌 문자열로 시작되는지 확인하는 메서드
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            res.status(401).json({ message: "로그인이 필요한 서비스 입니다. (토큰없음)" });
            return;
        }

        // 아직 판별만 진행했음, 여전히 authHeader = "Bearer 뭐뭐뭐"
        // split() 메서드 : string 타입에서 사용 가능. 매개변수에 집어 넣어준 문자열로 값을 나누는 메서드
        ...
        if (error instanceof jwt.TokenExpiredError) {
            res.status(401).json({ message: "토근이 만료 되었습니다. 다시 로그인 해주세요" });
            return;
        }
        if (error instanceof jwt.JsonWebTokenError) {
            res.status(401).json({
                message: "유효하지 않은 토큰 형식입니다. 다시 로그인 해주세요",
            });
            return;
        }
        console.error(error);
...
export const checkUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            // 헤더를 봤더니 내용이 없어? 그럼 그냥 컨트롤러로 가
            return next();
        }
        const token = authHeader.split(" ")[1];
        if (!token) {
            // 헤더에 Authorization 항목이 있는데 "Bearer <토큰이>" 이형식이 아니야 그럼 그냥 컨트롤러로 가
            return next();
        }
        const decoded = jwtUtil.verifyToken(token); // decoded = { id: ?}
        ...
        if (!user || user.deletedAt) {
            // 가지고 온 토튼일 검증하고 사용자 정보를 확인했는데 없어? 그럼 그냥 컨트롤러로 가
            return next();
        }
        req.user = user;
...
export const requiredAdmin = (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    // requiredAdmin이라는 함수는 다른 정보에는 접근이 불가능하지만, 매개변수로 들어오는 req, res, next는 쓸수 있음
    // 근데 res는 밖으로 나갈 박스니깡 여기서 낙서를 하면 안되고
    // next는 다름으로 진행하는 기능(메서드)니까, 당연히 메모는 안되고
    // 그럼 req는 외부에서 들어오는 내용이 담기는 박스이지만 ,메모는 상관 없다.
